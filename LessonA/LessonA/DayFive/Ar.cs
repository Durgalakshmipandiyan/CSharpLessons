...
namespace LessonA.DayFive
{
    internal class Ar
    {
        public static void TestArrayList()
        {
            ...
            //an enumerator is used to traverse a collection in a forward-only, read-only manner.
            if (alist is IEnumerable)//does alist impliment IEnumerable . looping over generic or non generic list.
                //Enumerator is useful for setting the next element
            {
                ...
                while (er.MoveNext())
                {
                    ...
                    {
                        //it checks the type of the current object obtained from the enumerator. If the type is int,
                        //it unboxes the object (converting from object to int) and prints it.
                        //If the type is not int, it prints the object as a string using ToString()
        ...
        public static void GenericListDemoEmp()
        {
            ...
            //}
            foreach (Emp e2 in empList) //e2 cn be any variable name
            {
